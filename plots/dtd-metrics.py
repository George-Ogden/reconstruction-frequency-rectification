...
def main(args):
    plotting_data = []
    for filename in glob("VQVAE/results/dtd*/metrics*.txt"):
        match = re.match(r"VQVAE/results/.+((w_wave)|(resnet\d+)|(j|w|(wo))_ffl)", filename)
        if match:
            model = match.group(1)
    ...
    order = ["resnet18","resnet34","resnet50", "resnet101", "resnet152", "w_wave", "wo_ffl", "w_ffl", "j_ffl"]
